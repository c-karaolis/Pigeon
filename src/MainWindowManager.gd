...
func _on_PostHTTPRequest_request_completed(_result: int, _response_code: int, _headers: PoolStringArray, _body: PoolByteArray):
#	var json = JSON.parse(body.get_string_from_utf8())
#	var texter = json.error if json.error != OK else json.result
	result_text_box.text = _body.get_string_from_utf8()


func _on_GetHTTPRequest_request_completed(_result: int, _response_code: int, _headers: PoolStringArray, _body: PoolByteArray):
	result_text_box.text = _body.get_string_from_utf8()


func _on_PutHTTPRequest_request_completed(_result: int, _response_code: int, _headers: PoolStringArray, _body: PoolByteArray):
	result_text_box.text = _body.get_string_from_utf8()
	pass 


func _on_PostButton_button_up():
	var user_input: Dictionary = get_refreshed_textbox_values()
	var body_text: String = user_input.RequestBody
	set_result_requesting_text()
#	var headers = ["Content-Type: application/json; charset=UTF-8"]
	body_text = remove_json_trailing_artifacts(body_text)
	post_request.request(user_input.URL, user_input.Headers, false, HTTPClient.METHOD_POST, body_text)


func _on_GetButton_button_up():
	
	get_request.request(url_box.text)


func _on_PutButton_button_up():
	pass # Replace with function body.
